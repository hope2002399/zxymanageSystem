...
InitInput = {
	...
	setNumber : function(obj,maxIntLength,maxDecimalLength,displayDecimal,attributes,allowNegative)
	{
	   ...
	   var initValue = function()//初始化对象的值
	   {
		...
		obj.onblur = function()//失去焦点时检查并格式化输入的数字
		{
			initValue();
		...
		obj.onkeypress = function() {//点击键盘按键时控制输入的字符为规定字符
		    initValue();
		...
		obj.onkeyup = function() {//按键弹起时检查输入的字符串长度是否符合要求
			...
			if(allowNegative)
				...
				regex=/^\d*?\.?\d*?$/;
			if(!obj.value.match(regex)) obj.value = obj.old_value;//如果字符串非数字 则还原成旧值
	...
	setInt : function(obj,len,attributes) 
	{
	   ...
	   var initValue = function()
	   {
		...
		obj.onkeypress = function() {
		    initValue();
			forbid();
			if(!obj.value.match(/^[\-]?\d*?\.?\d*?$/) || obj.value.length > len) obj.value = obj.t_value;
